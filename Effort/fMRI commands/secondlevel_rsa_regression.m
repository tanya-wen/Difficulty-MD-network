...
model_type = 'LDC';
l1dir = sprintf('/media/tw260/X6/Effort/analysis/rsa_analysis/%s_distance/Searchlight_10mm/Regression/',model_type);

spm('Defaults','FMRI');

analysisdir = sprintf('/media/tw260/X6/Effort/analysis/rsa_analysis/%s_distance/Searchlight_10mm/Regression/SecondLevel',model_type);
try cd(analysisdir)
...
for m = 1:numel(model_names)
    try
        ...
        for s = 1:numel(subject_list)
            V = spm_vol(sprintf('/media/tw260/X6/Effort/analysis/rsa_analysis/LDC_distance/Searchlight_10mm/Regression/%s_rsa_regression_%s.nii',subject_list{s},model_names{m}));
            vol = spm_read_vols(V);
            ...
            spm_smooth(map_unsmoothed,sprintf('%s/s10FWHM_%s',S.outdir,sprintf('%s_rsa_regression_%s.nii',subject_list{s},model_names{m})),[10,10,10]);
            
            S.imgfiles{1}{s} = fullfile(sprintf('%s/s10FWHM_%s',S.outdir,sprintf('%s_rsa_regression_%s.nii',subject_list{s},model_names{m})));
