    ...
    while subject_is_good
        for run = 1:5
            ...
            for trial = 1:data_orig.ntrials
                ...
                elseif data.seq_order(trial-1) > 3 && data.seq_order(trial) <= 3
                    idx(end+1) = 2; %'hard_easy';
                elseif data.seq_order(trial-1) > 3 && data.seq_order(trial) > 3
                    idx(end+1) = 3; %'hard_hard';
                elseif data.seq_order(trial-1) <= 3 && data.seq_order(trial) > 3
                    idx(end+1) = 4; %'easy_hard';
                end
            end
...
fig_path = '/media/tw260/X6/Effort/behav';
%% reaction time
...
t = array2table(anova_rt,'VariableNames',{'EE','HE','HH','EH'});
within = table(['r','s','r','s']',['e','e','h','h']','VariableNames',{'switchtype','difficulty'});
rm = fitrm(t,'EE,HE,HH,EH ~1','WithinDesign', within);
ranovatable = ranova(rm,'WithinModel','switchtype*difficulty');
...
t = array2table(anova_acc,'VariableNames',{'EE','HE','HH','EH'});
within = table(['r','s','r','s']',['e','e','h','h']','VariableNames',{'switchtype','difficulty'});
rm = fitrm(t,'EE,HE,HH,EH ~1','WithinDesign', within);
ranovatable = ranova(rm,'WithinModel','switchtype*difficulty');
