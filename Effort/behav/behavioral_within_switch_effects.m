    ...
    while subject_is_good
        for run = 1:5
            ...
            for trial = 1:data_orig.ntrials
                if trial == 1
                    idx(end+1) = 0;
                elseif data.seq_order(trial-1) < 3 && data.seq_order(trial) == 3 % within easy: switch to high
                    idx(end+1) = 1; %'switch_to_3';
                elseif data.seq_order(trial-1) > 4 && data.seq_order(trial) == 4 % within hard: switch to low
                    idx(end+1) = 2; %'switch_to_4';
                else
                    idx(end+1) = 0; %'other';
                end
            end
            ...
            try
                ...
                trial_ind = setdiff(acc_ind,find(idx==0)); % make sure not to include 1st trial
                response_time = data.response_time;
                func = @(x) median(x);
                rt_median(subj_ind,run,:) = splitapply(func, response_time(trial_ind), idx(trial_ind));
            catch
                rt_median(subj_ind,run,:) = NaN;
            end
            
            % accuracy
            response_acc = data.response_acc;
            func = @(x) mean(x);
            acc_mean(subj_ind,run,:) = splitapply(func, response_acc(idx~=0), idx(idx~=0));
...
bar(xx,squeeze(nanmean(nanmean(rt_median,2),1)),'FaceColor',[200, 200, 200]/255);
errorbar(xx,squeeze(nanmean(nanmean(rt_median,2),1)),squeeze(std(nanmean(rt_median,2),1))/sqrt(subj_ind-1), 'k.')
xticks(xx)
xticklabels({'switch to high', 'switch to low'})
...
set(gcf,'color','w');
set(gca,'box','off')
print(gcf,fullfile(fig_path,'within_switch_RT.eps'),'-depsc2','-painters');
comparisons_between_bars(xx, squeeze(nanmean(rt_median,2)))

cond_vals = squeeze(nanmean(rt_median,2));
[H,P,CI,STATS] = ttest(cond_vals(:,1),cond_vals(:,2))
...
set(gcf,'color','w');
set(gca,'box','off')
print(gcf,fullfile(fig_path,'within_switch_ACC.eps'),'-depsc2','-painters');
comparisons_between_bars(xx, squeeze(mean(acc_mean,2)))

cond_vals = squeeze(mean(acc_mean,2));
[H,P,CI,STATS] = ttest(cond_vals(:,1),cond_vals(:,2))